...
shm_address chunk_cache::allocate(size_t bytes, u8 sc) {
    assert_retval(sc < array_len(caches_), nullptr);

    // FIXME: chunk_cache is in shm_mgr's segment, which is not inside
    // a shm block, so we cannot get a valid shm_address here
    ...
    do {
        ...
        if (!span_list_empty(head)) {
            ...
            sp = head.as<span>()->next_span;
            s = sp.as<span>();
            break;
        }

        // no available chunk, then we fetch a span from page heap
        size_t page_count = shm_mgr::size_map()->class2pages(sc);
        sp = shm_mgr::page_heap()->allocate_span(page_count);
        ...
        stat_.total_size += (page_count << shm_config::PAGE_SHIFT);
        shm_mgr::page_heap()->register_span(sp);
        s->partition(bytes, sc);
        span_list_prepend(head, sp);
        ++caches_[sc].span_count;
        caches_[sc].stat.total_size += (page_count << shm_config::PAGE_SHIFT);
    } while (0);

    assert_retval(s->chunk_list, nullptr);

    shm_address ret = s->chunk_list;
    s->chunk_list = *(s->chunk_list.as<shm_address>());
    s->used_count += 1;
...
void chunk_cache::deallocate(shm_address addr) {
    assert_retnone(addr);

    shm_address sp = shm_mgr::page_heap()->find_span(addr);
    assert_retnone(sp);
    ...
    *(addr.as<shm_address>()) = s->chunk_list;
    s->chunk_list = addr;
    s->used_count -= 1;
    ...
    if (!full_before) {
        if (empty_after) {
            ...
            if (caches_[sc].span_count > 1) {
                ...
                const size_t bytes = (s->page_count << shm_config::PAGE_SHIFT);
                ...
                s->erase();
                shm_mgr::page_heap()->deallocate_span(sp);
            } else {
    ...
    const size_t bytes = shm_mgr::size_map()->class2size(sc);
